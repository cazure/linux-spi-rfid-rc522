 ...
 *
 *       Filename:  rc522.c
 *    Description:  This file contains the rc522 basic operations, include the 
 *                  read/write data and register, remove and set up a register
 *                  bit, calculate CRC16, rc522 and ISO 14443 CARDS, looking 
 *                  for card function, anticoll function, selected card function,
 *                  key authentication, write data to the M1 card of a block,
 *                  reset rc522 function, closed and open antenna.
 *        Version:  1.0.0(02/19/2014)
 *         Author:  fulinux <fulinux@sina.com>
 *      ChangeLog:  1, Release initial version on "02/19/2014 03:08:16 AM"
 *                 
 *************************************************************************************/
 ...
 * Return Value:
 *************************************************************************************/
void spi_read_byte (unsigned char *byte)
{
    int status;

    memset(byte, 0, sizeof(unsigned char));
...
} /* ----- End of unsigned char spi_read_byte()  ----- */
...
void spi_write_byte (unsigned char byte)
{
    ...
    status = write(fd, &byte, 1);
    if(status < 0)
    {
        pabort("can't write byte to device");
    }

} /* ----- End of spi_write_byte()  ----- */
